        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center px-2">
                <div className="flex items-center gap-2">
                    <div className="p-2.5 bg-orange-500/10 border border-orange-500/20 text-orange-400 rounded-2xl shadow-[0_0_15px_rgba(249,115,22,0.15)]">
            ...
            {ordersWithKitchen.length === 0 ? (
                <div className="py-24 text-center glass rounded-[3rem] p-10 border-dashed border-2 border-white/5 bg-gradient-to-br from-orange-500/5 to-transparent shadow-xl">
                    <div className="bg-slate-950/80 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6 border border-white/5 shadow-inner">
                        <CheckCircle2 size={44} className="text-orange-500 animate-bounce" />
                    </div>
                ...
                <div className="grid gap-5">
                    {ordersWithKitchen.map(order => {
                        ...
                        return (
                            <div key={order.id} className="glass rounded-[2rem] overflow-hidden border border-white/5 shadow-2xl hover:border-orange-500/20 transition-all duration-300 bg-gradient-to-br from-orange-500/5 to-rose-500/5">
                                <div className="p-5 bg-slate-950/70 flex justify-between items-center border-b border-white/5">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-orange-500 to-rose-600 flex items-center justify-center text-white shadow-[0_0_15px_rgba(249,115,22,0.2)] border border-white/10 shrink-0">
                                            <User size={20} />
                                        </div>
                                        <div>
                                            <h3 className="font-black text-lg text-white leading-none mb-1">{getOrderDisplayName(order, tenant)}</h3>
                                            <div className="flex items-center gap-2">
                                                <span className="text-[9px] text-orange-400 font-black tracking-widest uppercase">Orden #{order.order_number}</span>
                                                {order.table_number && (
                                                ...
                                                {order.origin === 'rappi' && (
                                                    <span className="text-[7.5px] bg-orange-500 text-slate-950 px-2 py-0.5 rounded-full font-black uppercase shadow-[0_0_10px_rgba(249,115,22,0.5)]">
                                                        RAPPI
                                                    </span>
                                                )}
                                    ...
                                    <div className="flex flex-col items-end gap-1">
                                        ...
                                        <button 
                                            ...
                                            className="text-[8px] font-black uppercase text-orange-400 hover:text-orange-300 bg-orange-500/10 px-2 py-1 rounded-lg border border-orange-500/20 hover:bg-orange-500/20 active:scale-95 transition-all mt-1"
                                        >
                                            Tildar Todo
                                        </button>
                                    </div>
                                            ...
                                            return (
                                                <div 
                                                    ...
                                                    className={`flex justify-between items-center p-3 rounded-2xl border transition-all duration-300 ${
                                                        isUpdating ? 'cursor-not-allowed opacity-70 border-orange-500/20' : 'cursor-pointer'
                                                    } ${
                                                        isDelivered 
                                                            ? 'bg-slate-950/20 border-white/5 opacity-40 hover:opacity-60' 
                                                            : 'bg-slate-950/60 border-white/10 hover:border-orange-500/30 hover:bg-slate-950/80 shadow-md'
                                                    }`}
                                                >
                                                    ...
                                                    <div className={`w-6 h-6 rounded-lg flex items-center justify-center transition-all ${
                                                        ...
                                                        {isUpdating ? (
                                                            <RefreshCw size={12} className="stroke-[3] animate-spin text-orange-400" />
                                                        ) : (
                                        ...
                                        <button
                                            ...
                                            className={`w-full font-black py-4.5 rounded-2xl shadow-xl transition-all active:scale-95 text-xs uppercase tracking-widest ${
                                                kitchenItems.every(i => {
                                                    ...
                                                    : 'bg-orange-500 hover:bg-orange-400 text-slate-950 shadow-orange-500/20 cursor-pointer'
                                            }`}
                                        >
                                            Tildar Todo
                                        </button>
