...
const AdminTab: React.FC<AdminTabProps> = ({
    ...
    const [expandedCustomizationSection, setExpandedCustomizationSection] = useState<string | null>(null);
    ...
    const isViewLocked = (v: string): boolean => {
        ...
        switch (v) {
            ...
            case 'reports':
                return !planFeatures.includes('Balance Financiero Avanzado');
            case 'rockola':
                return !planFeatures.includes('Muro Interactivo');
            default:
                return false;
        }
    };
    ...
    const [stkUnit, setStkUnit] = useState('unidades');
    ...
    useEffect(() => {
        if (tenant && (!isInitializedRef.current || isInitializedRef.current !== tenant.id)) {
            ...
            setCfgStaffPassword(tenant.staff_password || '');
            setCfgKitchenPassword(tenant.kitchen_password || '');
            setCfgDeliveryPassword(tenant.delivery_password || '');
            setCfgBartenderPassword(tenant.bartender_password || '');
            setCfgWaiterPassword(tenant.waiter_password || '');
            ...
            setCfgMercadopagoAccessToken(tenant.mercadopago_access_token || '');
    ...
    const handleSaveCategory = async () => {
        ...
        if (editingCategoryId) {
            const { error } = await supabase.from('categories').update({ 
                name: catName, 
                icon: catIcon, 
                target_departments: catTargetDepartments,
                is_offer: catIsOffer,
    ...
    const handleSaveStock = async () => {
        ...
        if (diff > 0 && newUnitPrice > 0) {
            try {
                ...
                const description = isNew
                    ? `Ingreso inicial de Insumo: ${stkName} (x${formatStock(diff)} ${stkUnit})`
                    : `Compra de Insumo: ${stkName} (x${formatStock(diff)} ${stkUnit})`;
        ...
        if (diff < 0) {
            try {
                ...
                const description = `Merma: ${stkName} - ${wasteReason} (x${formatStock(Math.abs(diff))} ${stkUnit})`;
        ...
        setStkName(''); setStkPrice(''); setStkLevel(''); setStkUnit('unidades'); setStkMinAlert('10'); setStkTargetDepartments(['kitchen']);
    ...
    const handleSaveWaste = async () => {
        ...
        try {
            ...
            const description = `Merma: ${selectedWasteIngredient.name} - ${wasteReason} (x${formatStock(qty)} ${selectedWasteIngredient.unit})`;
        ...
        <div className="space-y-6 pb-4 max-w-5xl mx-auto px-2">
            ...
            {subscription && (
                <div className="animate-in fade-in slide-in-from-top-2">
                    {(() => {
                        ...
                        const isTrialPending = (!trialStartedAt && !isPromoActive) && (subscription.status === 'pending_trial' || subscription.status === 'trial');
                                ...
                                <div className="bg-slate-800/80 border border-slate-700/50 p-4 rounded-2xl flex items-center justify-between text-slate-300">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs">Tus 14 días gratuitos de <strong className="text-purple-400">Plan Pro</strong> comenzarán automáticamente cuando registres tu primera venta real.</p>
                                        </div>
                                    </div>
                                </div>
                            );
            ...
            {view === 'dashboard' && (
                <div className="space-y-6 animate-in fade-in">
                    ...
                    <div className="glass p-6 rounded-[2.5rem] border border-white/5">
                        <h4 className="text-[10px] font-black uppercase text-orange-500 mb-4">Lo más vendido del mes</h4>
                        <div className="space-y-3">
                                ...
                                <div key={item.id} className="glass rounded-3xl border border-white/5 overflow-hidden">
                                    ...
                                    {lotesDeEsteIngrediente.length > 0 && (
                                        <div className="bg-slate-950/40 border-t border-white/5 p-3 space-y-2">
                                            <p className="text-[8px] font-black text-orange-500/80 uppercase tracking-widest flex items-center gap-1">
                                                        ...
                                                        <div key={batch.id} className="bg-slate-900/50 px-3 py-2 rounded-xl flex justify-between items-center border border-white/5">
                                                            <div className="flex items-center gap-2">
                                                                <div className={`w-2.5 h-2.5 rounded-full ${
                                                                    ...
                                                                    diffDays <= 7 ? 'bg-orange-500 shadow-[0_0_6px_rgba(249,115,22,0.4)]' : 'bg-green-500'
                                                                }`} />
                                                                <div>
                                                                    <p className="text-[10px] font-black text-slate-200">
                                                                        {formatStock(batch.quantity)} {item.unit}
                                                                    </p>
                                                            ...
                                                            <button
                                                                onClick={async (e) => {
                                                                    ...
                                                                    const confirmDelete = window.confirm(
                                                                        `⚠️ ¿Deseas eliminar este lote de ${formatStock(batch.quantity)} ${item.unit} de "${item.name}" con fecha de vencimiento ${formattedDate}?`
                                                                    );
                                                                    if (confirmDelete) {
                                                                        const { error } = await supabase.from('ingredient_batches').delete().eq('id', batch.id);
                                                                        if (error) {
                    ...
                    <div className="flex justify-between items-center px-2 flex-wrap gap-2">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                ...
                                className="bg-orange-500 hover:bg-orange-600 px-3.5 py-2 rounded-xl text-white text-[10px] font-black uppercase tracking-wider shadow-lg transition-all active:scale-95">
                                Nueva Categoría
                            </button>
                        </div>
                    </div>
                                        ...
                                        <div key={offer.id} className="bg-slate-950/40 p-3 rounded-2xl border border-white/5 flex justify-between items-center gap-3">
                                            <div className="space-y-1 flex-1">
                                                ...
                                                {offer.limit_quantity && (
                                                    <p className="text-[8px] text-orange-500/80 font-black uppercase">
                        ...
                        return (
                            <div key={cat.id} className="space-y-3 bg-slate-900/30 p-4 rounded-[2rem] border border-white/5">
                                <div className="flex justify-between items-center border-b border-white/5 pb-2 mb-2">
                                    <h4 className="font-black text-orange-500 flex items-center gap-2 text-sm">
                                            ...
                                            <div className="flex items-center gap-3">
                                                {prod.is_active !== false && (
                                                    <input
                                                        ...
                                                        className="w-3.5 h-3.5 rounded bg-slate-950 border border-slate-800 text-orange-500 focus:ring-orange-500/25 cursor-pointer accent-orange-500"
                                                    />
                                                )}
                                        ...
                                        <div className="p-5 space-y-4">
                                            <div className="grid grid-cols-3 gap-2">
                                                ...
                                                <div 
                                                    ...
                                                    className={`p-2.5 rounded-2xl border flex flex-col justify-between cursor-pointer active:scale-[0.98] transition-all hover:bg-orange-500/5 ${
                                                        expandedSection?.month === month && expandedSection?.type === 'waste'
                                                            ? 'border-orange-500 bg-orange-500/10 ring-1 ring-orange-500'
                                                            : 'bg-slate-950/40 border-orange-500/15'
                                                    }`}
                                                >
                                                    <div className="flex items-center gap-1.5 mb-1">
                                            ...
                                            return (
                                                <div className="bg-slate-950/40 p-4 rounded-2xl border border-white/5 space-y-2">
                                                    <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1">
                                                        <Trophy size={10} className="text-orange-500 animate-pulse" />
                                                        Top 3 Más Vendidos del Mes
                                                    </p>
                                                    <div className="grid grid-cols-3 gap-2">
                                                        {stats.map((item, i) => {
                                                        ...
                                                        return (
                                                            <div key={iid} className="space-y-1">
                                                                <div className="flex justify-between items-center text-xs">
                                                                    ...
                                                                    <span className="font-black text-cyan-500">{formatStock(qty)} {ing?.unit}</span>
                                                                </div>
                                                                <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                        ...
                        <div className="space-y-4 animate-in slide-in-from-top-4 duration-300">
                            ...
                            >
                                ...
                                <div className="space-y-1">
                                    <p className="text-[9px] font-black uppercase tracking-widest text-orange-200/80">Ventas Totales Facturadas</p>
                                    <h2 className="text-3xl font-black text-white font-mono tracking-tight">{formatARS(dailyStats.total)}</h2>
                                </div>
                                <p className="text-[8px] font-black text-orange-100/50 uppercase mt-4">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                ...
                                {/* Plataformas */}
                                <div className="glass p-4 rounded-3xl border border-orange-500/10 bg-slate-950/40 relative overflow-hidden">
                                            ...
                                            >
                                                <div className="text-left">
                                                    <p className="font-black text-sm text-white">
                                                        <span className="text-orange-500 mr-1.5">#{order.order_number || '?'}</span>
                                                        {order.client_name}
                                                    </p>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <div className="text-right">
                                                        <p className="font-black text-orange-500 font-mono text-xs">{formatARS(order.total_price)}</p>
                                                    ...
                                                    <ChevronRight 
                                                        size={12} 
                                                        className={`text-slate-500 transition-transform duration-300 ${expandedModalOrderId === order.id ? 'rotate-90 text-orange-500' : ''}`} 
                                                    />
                                                </div>
                                            </div>
                                ...
                                <div className="flex justify-between items-center px-2">
                                    ...
                                    <div className="flex items-center gap-1.5 bg-slate-900 border border-slate-800/80 px-2.5 py-1 rounded-[1rem]">
                                        ...
                                        <select
                                            ...
                                            className="bg-transparent text-[8px] font-black uppercase text-orange-500 outline-none cursor-pointer border-none"
                                        >
                                    ...
                                    return (
                                        <div key={month} className="glass rounded-3xl border border-white/5 overflow-hidden">
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <span className="font-mono font-black text-orange-500 text-sm">{formatARS(monthTotal)}</span>
                                                    <ChevronRight 
                                                        size={16} 
                                                        className={`text-slate-500 transition-transform duration-300 ${isMonthExpanded ? 'rotate-90 text-orange-500' : ''}`} 
                                                    />
                                                </div>
                                            </div>
                                                                                                ...
                                                                                                >
                                                                                                    <div>
                                                                                                        <p className="font-black text-[11px]">
                                                                                                            <span className="text-orange-500 mr-1">#{order.order_number || '?'}</span>
                                                                                                            {order.client_name}
                                                                                                        </p>
                                                                                                    ...
                                                                                                    <div className="flex items-center gap-1.5">
                                                                                                        <span className="font-mono font-black text-orange-500 text-[11px]">{formatARS(order.total_price)}</span>
                                                                                                        <ChevronRight 
                                                                                                            size={10} 
                                                                                                            className={`text-slate-500 transition-transform duration-300 ${expandedOrderId === order.id ? 'rotate-90 text-orange-500' : ''}`} 
                                                                                                        />
                                                                                                    </div>
                                                                                                </div>
            ...
            {view === 'config' && (
                <div className="space-y-6 animate-in slide-in-from-bottom-4">
                    ...
                    <div className="space-y-4">
                        {/* Accordion: Personalización del Local */}
                        <div className="flex flex-col gap-2">
                            <button
                                onClick={() => setExpandedConfigSection(prev => prev === 'personalizacion' ? null : 'personalizacion')}
                                className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                                    true 
                                      ? (expandedConfigSection === 'personalizacion' ? 'bg-orange-500/10 border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.1)]' : 'bg-slate-900/80 border-orange-500/30 text-orange-400')
                                      : (expandedConfigSection === 'personalizacion' ? 'bg-slate-800 border-slate-600 text-white' : 'bg-slate-950 border-slate-800 text-slate-500 opacity-80')
                                }`}
                                style={{
                                    borderColor: true ? (expandedConfigSection === 'personalizacion' ? tenant?.theme_colors?.primary : undefined) : undefined,
                                    color: true ? (expandedConfigSection === 'personalizacion' ? tenant?.theme_colors?.primary : undefined) : undefined,
                                }}
                            >
                                <div className="flex items-center gap-3">
                            ...
                            {expandedConfigSection === 'personalizacion' && (
                        ...
                        <div className="flex flex-col gap-2">
                            <button
                                onClick={() => setExpandedCustomizationSection(prev => prev === 'identidad' ? null : 'identidad')}
                                className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                                    true 
                                      ? (expandedCustomizationSection === 'identidad' ? 'bg-orange-500/10 border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.1)]' : 'bg-slate-900/80 border-orange-500/30 text-orange-400')
                                      : (expandedCustomizationSection === 'identidad' ? 'bg-slate-800 border-slate-600 text-white' : 'bg-slate-950 border-slate-800 text-slate-500 opacity-80')
                                }`}
                                style={{
                                    borderColor: true ? (expandedCustomizationSection === 'identidad' ? tenant?.theme_colors?.primary : undefined) : undefined,
                                    color: true ? (expandedCustomizationSection === 'identidad' ? tenant?.theme_colors?.primary : undefined) : undefined,
                                }}
                            >
                                <div className="flex items-center gap-3">
                                    <Paintbrush className="w-5 h-5" />
                                ...
                                {expandedCustomizationSection === 'identidad' ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                            </button>
                            {expandedCustomizationSection === 'identidad' && (
                                <div className="glass p-6 rounded-[2.5rem] border border-white/5 space-y-5 animate-in slide-in-from-top-2">
                        {/* 1. Tema de Colores */}
                        <div className="space-y-3">
                        ...
                        <div className="space-y-2 pt-1">
                            ...
                            <div className="flex gap-2">
                                ...
                                >
                                    <Sun size={12} className={cfgMode === 'light' ? 'text-orange-500' : ''} /> Tema Claro
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-2">
                            <button
                                onClick={() => setExpandedCustomizationSection(prev => prev === 'business_hours' ? null : 'business_hours')}
                                className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                                    cfgBusinessHours.enabled 
                                      ? (expandedCustomizationSection === 'business_hours' ? 'bg-orange-500/10 border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.1)]' : 'bg-slate-900/80 border-orange-500/30 text-orange-400')
                                      : (expandedCustomizationSection === 'business_hours' ? 'bg-slate-800 border-slate-600 text-white' : 'bg-slate-950 border-slate-800 text-slate-500 opacity-80')
                                }`}
                                style={{
                                    borderColor: cfgBusinessHours.enabled ? (expandedCustomizationSection === 'business_hours' ? tenant?.theme_colors?.primary : undefined) : undefined,
                                    color: cfgBusinessHours.enabled ? (expandedCustomizationSection === 'business_hours' ? tenant?.theme_colors?.primary : undefined) : undefined,
                                }}
                            >
                                <div className="flex items-center gap-3">
                                ...
                                {expandedCustomizationSection === 'business_hours' ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                            </button>
                            {expandedCustomizationSection === 'business_hours' && (
                                <div className="glass p-6 rounded-[2.5rem] border border-white/5 space-y-5 animate-in slide-in-from-top-2">
                        ...
                        <div className="flex flex-col gap-2">
                            <button
                                onClick={() => setExpandedCustomizationSection(prev => prev === 'social' ? null : 'social')}
                                className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                                    true 
                                      ? (expandedCustomizationSection === 'social' ? 'bg-orange-500/10 border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.1)]' : 'bg-slate-900/80 border-orange-500/30 text-orange-400')
                                      : (expandedCustomizationSection === 'social' ? 'bg-slate-800 border-slate-600 text-white' : 'bg-slate-950 border-slate-800 text-slate-500 opacity-80')
                                }`}
                                style={{
                                    borderColor: true ? (expandedCustomizationSection === 'social' ? tenant?.theme_colors?.primary : undefined) : undefined,
                                    color: true ? (expandedCustomizationSection === 'social' ? tenant?.theme_colors?.primary : undefined) : undefined,
                                }}
                            >
                                <div className="flex items-center gap-3">
                                ...
                                {expandedCustomizationSection === 'social' ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                            </button>
                            {expandedCustomizationSection === 'social' && (
                                <div className="glass p-6 rounded-[2.5rem] border border-white/5 space-y-5 animate-in slide-in-from-top-2">
                            ...
                            <div className="space-y-4 bg-slate-950/40 p-4 rounded-2xl border border-white/5 text-left">
                                ...
                                <div className="space-y-1 mt-3">
                                    ...
                                    <textarea
                                        ...
                                        rows={3}
                                        maxLength={500}
                                        className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-white text-xs outline-none focus:border-orange-500/50 resize-none"
                                ...
                                {/* Instagram */}
                                <div className="space-y-1">
                                    <label className="text-[8px] font-bold uppercase text-slate-500 block ml-1 flex items-center gap-1"><Instagram size={10} className="text-orange-500" /> Enlace de Instagram</label>
                                    <input
                                        type="text"
                                        value={cfgInstagram}
                                ...
                                {/* Facebook */}
                                <div className="space-y-1">
                                    <label className="text-[8px] font-bold uppercase text-slate-500 block ml-1 flex items-center gap-1"><Facebook size={10} className="text-orange-500" /> Enlace de Facebook</label>
                                    <input
                                        type="text"
                                        value={cfgFacebook}
                                ...
                                <div className="space-y-1">
                                    <label className="text-[8px] font-bold uppercase text-slate-500 block ml-1 flex items-center gap-1"><Phone size={10} className="text-orange-500" /> WhatsApp / Celular de Contacto</label>
                                    <input
                                        type="text"
                                        value={cfgWhatsapp}
                                ...
                                <div className="space-y-1">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-white text-xs outline-none focus:border-orange-500/50 min-h-[60px]"
                                ...
                                <div className="pt-2 flex items-center justify-between">
                                    ...
                                    <button
                                        ...
                                        className={`relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                                            cfgReviewsEnabled ? 'bg-orange-500' : 'bg-slate-800'
                                        }`}
                                    >
                                        <span
                        ...
                        <div className="flex flex-col gap-2">
                            <button
                                onClick={() => setExpandedCustomizationSection(prev => prev === 'reservas' ? null : 'reservas')}
                                className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                                    cfgReservationsEnabled 
                                      ? (expandedCustomizationSection === 'reservas' ? 'bg-orange-500/10 border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.1)]' : 'bg-slate-900/80 border-orange-500/30 text-orange-400')
                                      : (expandedCustomizationSection === 'reservas' ? 'bg-slate-800 border-slate-600 text-white' : 'bg-slate-950 border-slate-800 text-slate-500 opacity-80')
                                }`}
                                style={{
                                    borderColor: cfgReservationsEnabled ? (expandedCustomizationSection === 'reservas' ? tenant?.theme_colors?.primary : undefined) : undefined,
                                    color: cfgReservationsEnabled ? (expandedCustomizationSection === 'reservas' ? tenant?.theme_colors?.primary : undefined) : undefined,
                                }}
                            >
                                <div className="flex items-center gap-3">
                                ...
                                {expandedCustomizationSection === 'reservas' ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                            </button>
                            {expandedCustomizationSection === 'reservas' && (
                                <div className="glass p-6 rounded-[2.5rem] border border-white/5 space-y-5 animate-in slide-in-from-top-2">
                        ...
                        {cfgReservationsEnabled && (
                            <div className="space-y-5 pt-2 animate-in slide-in-from-top-2 duration-300">
                                ...
                                {/* Listado de Reservas */}
                                <div className="bg-slate-950/40 border border-white/5 p-4 rounded-2xl space-y-3 text-left">
                                    <h3 className="text-[9px] font-bold uppercase text-slate-500">Reservas Activas</h3>
                                    {isResDataLoading ? (
                                        <p className="text-[10px] text-slate-500 italic">Cargando reservas...</p>
                                    ) : activeReservations.length === 0 ? (
                                        <p className="text-[10px] text-slate-500 italic">No hay reservas registradas.</p>
                                    ) : (
                                        <div className="space-y-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
                                            {activeReservations.map((res: any) => (
                                                <div key={res.id} className="bg-slate-900/60 p-3 rounded-xl border border-white/5 flex flex-col gap-2">
                                                    <div className="flex justify-between items-start">
                                                        <div>
                                                            <p className="text-xs font-bold text-white">{res.customer_name}</p>
                                                            <p className="text-[9px] text-slate-400">{res.reservation_date} a las {res.reservation_time} ({res.party_size} pers.)</p>
                                                            <p className="text-[9px] text-orange-400 font-mono mt-0.5">CÓDIGO: {res.reservation_code}</p>
                                                            {res.is_deposit_applied && <p className="text-[8px] text-red-400 font-bold uppercase mt-1">Seña Consumida</p>}
                                                        </div>
                                                        <span className={`px-2 py-1 rounded-md text-[8px] font-bold uppercase ${res.status === 'confirmed' ? 'bg-green-500/20 text-green-400' : res.status === 'cancelled' ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                                                            {res.status}
                                                        </span>
                                                    </div>
                                                    {res.customer_phone && (
                                                        <a 
                                                            href={`https://wa.me/${formatWhatsAppNumber(res.customer_phone)}?text=${encodeURIComponent(`¡Hola ${res.customer_name}! Te escribimos de ${tenant?.name || 'nuestro local'}. Queremos confirmarte tu reserva para el día ${res.reservation_date} a las ${res.reservation_time}.\n\nTu código de reserva es: ${res.reservation_code}\n\n⚠️ Tienes que guardar ese código de reserva, que es muy importante, y colocarlo (o dárselo al cajero) en el momento de pagar para que se te descuente la seña. ¡Te esperamos!`)}`}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="w-full py-1.5 bg-green-600/20 hover:bg-green-600/30 text-green-400 border border-green-500/30 rounded-lg text-[9px] font-bold flex items-center justify-center gap-1.5 transition-all"
                                                        >
                                ...
                                <div className="bg-slate-950/40 border border-white/5 p-4 rounded-2xl space-y-3 text-left">
                                    <h3 className="text-[9px] font-bold uppercase text-slate-500">Generar Código de Descuento</h3>
                                    <div className="grid grid-cols-2 gap-2">
                                        <input
                                            ...
                                            className="bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-white text-xs outline-none focus:border-orange-500/50 uppercase"
                                        />
                                        <input
                                            type="number"
                                            value={newCodeAmount}
                                            ...
                                            placeholder="Monto ($)"
                                            className="bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-white text-xs outline-none focus:border-orange-500/50"
                                        />
                                    </div>
                                    <input
                                        type="text"
                                        ...
                                        className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-white text-[10px] outline-none focus:border-orange-500/50"
                                    />
                                    <button
                                        type="button"
                                        onClick={handleCreateCode}
                                        disabled={isGeneratingCode || !newCodeName || !newCodeAmount}
                                        className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white text-[10px] font-bold rounded-xl shadow-lg transition-all"
                                    >
                                        {isGeneratingCode ? 'Generando...' : 'Crear Código'}
                                    </button>

                                    {/* Listado de Códigos */}
                                    <div className="pt-3 border-t border-white/5 space-y-2 mt-2">
                                        <h3 className="text-[9px] font-bold uppercase text-slate-500">Códigos Activos e Historial</h3>
                                        {isResDataLoading ? (
                                            <p className="text-[10px] text-slate-500 italic">Cargando códigos...</p>
                                        ) : discountCodes.length === 0 ? (
                                            <p className="text-[10px] text-slate-500 italic">No hay códigos de descuento.</p>
                                        ) : (
                                            <div className="space-y-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
                                                {discountCodes.map((c: any) => (
                                                    <div key={c.id} className="bg-slate-900/60 p-3 rounded-xl border border-white/5 flex justify-between items-center">
                                                        <div>
                                                            <p className={`text-xs font-bold ${c.is_used ? 'text-slate-500 line-through' : 'text-orange-400'}`}>{c.code}</p>
                                                            <p className="text-[9px] text-slate-400">Descuento: ${c.discount_amount}</p>
                                                            {c.description && <p className="text-[8px] text-slate-500 mt-0.5">{c.description}</p>}
                                                        </div>
                                                        <span className={`px-2 py-1 rounded-md text-[8px] font-bold uppercase ${c.is_used ? 'bg-red-500/10 text-red-500' : 'bg-green-500/10 text-green-500'}`}>
                                                            {c.is_used ? 'Usado' : 'Activo'}
                                                        </span>
                                                    </div>
                                                ))}
                                            </div>
                                ...
                                <div className="bg-slate-950/40 border border-white/5 p-4 rounded-2xl space-y-3 text-left">
                                    <h3 className="text-[9px] font-bold uppercase text-slate-500">Caja: Validar Código</h3>
                                    <div className="flex gap-2">
                                        <input
                                            type="text"
                                            ...
                                            className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-white text-xs outline-none focus:border-orange-500/50 uppercase"
                                        />
                                        <button
                                            type="button"
                                            onClick={handleValidateAdminCode}
                                            disabled={isAdminValidatingCode || !adminManualCode}
                                        ...
                                        >
                                            {isAdminValidatingCode ? 'Validando...' : 'Validar'}
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col gap-2">
                            <button
                                onClick={() => setExpandedCustomizationSection(prev => prev === 'landing' ? null : 'landing')}
                                className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                                    cfgLandingConfig.enabled 
                                      ? (expandedCustomizationSection === 'landing' ? 'bg-orange-500/10 border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.1)]' : 'bg-slate-900/80 border-orange-500/30 text-orange-400')
                                      : (expandedCustomizationSection === 'landing' ? 'bg-slate-800 border-slate-600 text-white' : 'bg-slate-950 border-slate-800 text-slate-500 opacity-80')
                                }`}
                                style={{
                                    borderColor: cfgLandingConfig.enabled ? (expandedCustomizationSection === 'landing' ? tenant?.theme_colors?.primary : undefined) : undefined,
                                    color: cfgLandingConfig.enabled ? (expandedCustomizationSection === 'landing' ? tenant?.theme_colors?.primary : undefined) : undefined,
                                }}
                            >
                                <div className="flex items-center gap-3">
                                ...
                                {expandedCustomizationSection === 'landing' ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                            </button>
                            {expandedCustomizationSection === 'landing' && (
                                <div className="glass p-6 rounded-[2.5rem] border border-white/5 space-y-5 animate-in slide-in-from-top-2">
                                    {/* Enable Landing Page */}
                        ...
                        <div className="space-y-3 pt-3 border-t border-white/5">
                            ...
                            <label className={`p-3.5 rounded-2xl border cursor-pointer flex items-center justify-between transition-all ${
                                cfgHasDelivery
                                    ? 'bg-slate-900/60 border-orange-500/30 text-white'
                ...
                <div className="space-y-6 animate-in slide-in-from-bottom-4">
                    ...
                    {loyConfigEnabled && (
                        ...
                        <div className="flex flex-col gap-2">
                            <button
                                onClick={() => {
                                    if (isViewLocked('rockola')) {
                                        setLockedFeatureModal('rockola');
                                        return;
                                    }
                            ...
                            {expandedConfigSection === 'rockola_vip' && !isViewLocked('rockola') && (
                                <div className="p-6 bg-slate-950/80 border border-purple-500/20 rounded-2xl animate-in slide-in-from-bottom-2 text-center space-y-4">
                                    ...
                                    <h3 className="text-lg font-black uppercase text-purple-400">Muro Interactivo (Próximamente)</h3>
                                    <p className="text-xs text-slate-400 font-medium">
                                        ¡Esta funcionalidad ya está activada en tu Plan Pro!<br/>
                                        Pronto podrás configurar las fotos, audios y videos en vivo para tu local.
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        type="button"
                                        onClick={() => setCatTargetDepartments(['kitchen'])}
                                        className={`flex-1 py-3 px-4 rounded-xl border text-[9px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 ${
                                            catTargetDepartments.includes('kitchen')
                                    ...
                                    {(!subscription || !['Plan Básico', 'Plan Intermedio'].includes(subscription?.saas_plans?.name)) && (
                                        <button
                                            ...
                                            className={`flex-1 py-3 px-4 rounded-xl border text-[9px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 ${
                                                catTargetDepartments.includes('bartender')
                                                    ? 'bg-slate-900 border-orange-500/30 text-white shadow-lg'
                                                    : 'bg-slate-950/20 border-slate-900 text-slate-500 hover:text-slate-400'
                                            }`}
                                        >
                            ...
                            <div className="space-y-2">
                                ...
                                >
                                    <Star size={12} className={catIsOffer ? 'fill-orange-500' : ''} />
                            ...
                            <button onClick={handleSaveCategory} className="w-full py-5 bg-orange-500 text-white font-black rounded-2xl shadow-[0_10px_25px_-5px_rgba(249,115,22,0.4)] uppercase tracking-widest active:scale-95 transition-all">
                                ...
                                <div className="glass p-4 rounded-2xl border border-purple-500/20 bg-purple-500/5 space-y-2">
                                    ...
                                    <div className="space-y-2 max-h-[140px] overflow-y-auto pr-1 custom-scrollbar">
                                        {offSelectedProducts.map(pid => {
                                            ...
                                            const marginColor = newMargin <= 20 ? 'text-red-500' : newMargin <= 50 ? 'text-orange-500' : 'text-green-500';
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
                    <div className="glass w-full max-w-sm rounded-[2.5rem] p-8 space-y-6 shadow-2xl border border-white/10">
                        ...
                        <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black uppercase text-slate-500 ml-2">Unidad (unidades, kg, l)</label>
                                    <input type="text" value={stkUnit} onChange={e => setStkUnit(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-2xl p-4 text-white font-bold outline-none" placeholder="unidades" />
                                </div>
                                <div className="space-y-1">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        type="button"
                                        onClick={() => setStkTargetDepartments(['kitchen'])}
                                        className={`flex-1 py-3 px-4 rounded-xl border text-[9px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 ${
                                            stkTargetDepartments.includes('kitchen')
                                                ? 'bg-slate-900 border-orange-500/30 text-white shadow-lg'
                                                : 'bg-slate-950/20 border-slate-900 text-slate-500 hover:text-slate-400'
                                        }`}
                                        style={stkTargetDepartments.includes('kitchen') ? { borderColor: `${tenant?.theme_colors?.primary || '#f97316'}40` } : {}}
                                    >
                                        🍳 Cocina
                                    </button>
                                    <button
                                        type="button"
                                        onClick={() => setStkTargetDepartments(['bartender'])}
                                        className={`flex-1 py-3 px-4 rounded-xl border text-[9px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 ${
                                            stkTargetDepartments.includes('bartender')
                                                ? 'bg-slate-900 border-orange-500/30 text-white shadow-lg'
                                                : 'bg-slate-950/20 border-slate-900 text-slate-500 hover:text-slate-400'
                                        }`}
                                        style={stkTargetDepartments.includes('bartender') ? { borderColor: `${tenant?.theme_colors?.primary || '#f97316'}40` } : {}}
                                    >
                                        🍹 Barra
                                    </button>
                                </div>
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-black uppercase text-slate-500 ml-2 flex items-center gap-1.5">
                                    <CalendarRange size={12} className="text-orange-500" />
                            ...
                            {editingStockId && parseFloat(stkLevel || '0') < (ingredients.find(i => i.id === editingStockId)?.stock_level || 0) && (
                                <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-2xl space-y-2 animate-in slide-in-from-top-4">
                                    ...
                                    <p className="text-[8.5px] text-slate-400 font-bold leading-normal">
                                        Estás reduciendo el stock de {formatStock(ingredients.find(i => i.id === editingStockId)?.stock_level || 0)} a {formatStock(parseFloat(stkLevel || '0'))} {stkUnit}. Esta diferencia se registrará automáticamente como un gasto por merma en el balance.
                                    </p>
                                    <div className="space-y-1">
                        ...
                        <div className="space-y-4">
                            <div className="bg-slate-950/40 p-4.5 rounded-2xl border border-white/5 space-y-1">
                                ...
                                <div className="flex justify-between items-center text-[10px] text-slate-400 mt-1">
                                    <span>Stock Actual: {formatStock(selectedWasteIngredient.stock_level)} {selectedWasteIngredient.unit}</span>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm animate-in fade-in overflow-y-auto pt-10 pb-10">
                    <div className="glass w-full max-w-sm rounded-[3rem] p-8 space-y-6 shadow-2xl border border-white/10 my-auto">
                        ...
                        <div className="space-y-6 max-h-[80vh] overflow-y-auto pr-2 custom-scrollbar">
                            ...
                            <div className="space-y-4">
                                ...
                                {(() => {
                                    ...
                                    const marginColor = marginPercent <= 20 
                                        ...
                                        : marginPercent <= 50 
                                            ? 'text-orange-500 border-orange-500/20 bg-orange-500/5 shadow-orange-500/5' 
                            ...
                            <button onClick={handleSaveProduct} className="w-full py-5 bg-orange-600 text-white font-black rounded-2xl shadow-xl uppercase tracking-widest sticky bottom-0">
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center px-4 bg-black/85 backdrop-blur-md animate-in fade-in">
                    <div className="glass w-full max-w-lg rounded-[2.5rem] p-6 space-y-6 shadow-2xl border border-white/10 flex flex-col max-h-[85vh] overflow-hidden">
                        ...
                        <div className="flex justify-between items-center pb-3 border-b border-white/5">
                            <div>
                                <h3 className="text-lg font-black uppercase italic text-orange-500">Resumen Diario</h3>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-5 pr-1 custom-scrollbar">
                            ...
                            >
                                <div className="space-y-0.5">
                                    <p className="text-[8px] font-black uppercase tracking-widest text-orange-200/80">Total del Día Seleccionado</p>
                                    <h2 className="text-2xl font-black text-white font-mono tracking-tight">{formatARS(selectedDayStats.total)}</h2>
                                </div>
                                <p className="text-[7px] font-black text-orange-100/50 uppercase mt-2">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                ...
                                <div className="glass p-3 rounded-2xl border border-orange-500/10 bg-slate-950/40 relative overflow-hidden">
                                    <div className="absolute top-1.5 right-1.5 w-1 h-1 rounded-full bg-orange-500" />
                ...
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[99] flex items-center justify-center p-4 animate-in fade-in duration-200">
                    <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-[2.5rem] p-6 space-y-6 shadow-2xl text-left animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center border-b border-white/5 pb-3">
                            <h3 className="font-black uppercase italic text-sm text-orange-500">Ajuste de Monedero Virtual</h3>
                            <button
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-slate-950/40 p-4 rounded-2xl border border-white/5">
                                ...
                                <span className={`text-[7px] uppercase font-black px-2 py-0.5 rounded-full ${
                                    ...
                                    editingLoyaltyAccount.tier === 'plata' ? 'bg-slate-300/10 text-slate-350' : 'bg-orange-500/10 text-orange-400'
                                }`}>
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-6 pr-4 py-2 text-xs text-white font-bold outline-none focus:border-orange-500/50"
                                    />
                                </div>
                        ...
                        <div className="flex gap-3 pt-2">
                            <button
                                ...
                                className="flex-1 py-3 bg-orange-500 text-[10px] font-black uppercase text-white rounded-xl active:scale-95 transition-all shadow-md text-center"
                                style={{ backgroundColor: tenant?.theme_colors?.primary || '#f97316' }}
                            >
                                Actualizar Saldo
                            </button>
            ...
            {lockedFeatureModal && (
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="glass max-w-sm w-full p-8 rounded-[2rem] border border-white/10 text-center relative overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <h3 className="text-lg font-black text-white uppercase tracking-wide">Función Pro 🔒</h3>
                        <p className="text-slate-400 text-xs mt-3 leading-relaxed">
                            {lockedFeatureModal === 'loyalty' && 'El Programa de Fidelización (Club de Clientes) con monedero virtual, cashback configurable y tiers Bronce/Plata/Oro está disponible de forma exclusiva en el Plan Pro.'}
                            ...
                            {lockedFeatureModal === 'balance' && 'El Balance Financiero con contabilidad integrada, gráficos interactivos de pérdidas y ganancias y reportes de rentabilidad está disponible en el Plan Pro.'}
                            {lockedFeatureModal === 'reports' && 'La exportación avanzada de auditoría y reportes detallados en CSV está disponible en el Plan Pro.'}
                            {lockedFeatureModal === 'rockola' && 'El Muro Interactivo, donde tus clientes pueden compartir fotos, audios y videos en vivo, está disponible exclusivamente en el Plan Pro.'}
                        </p>
