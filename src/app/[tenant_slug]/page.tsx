  ...
  return (
    <div className={`min-h-screen pb-28 transition-colors ${isLight ? 'bg-slate-50 text-slate-900' : 'bg-slate-950 text-white'}`}>
      ...
      {isSupported && !isSubscribed && profile.role !== 'admin' && (
        <div className="bg-orange-500 text-white px-4 py-2 text-xs font-bold flex justify-between items-center shadow-md">
          ...
          <button 
            ...
            className="bg-white text-orange-600 px-3 py-1 rounded-full text-[10px] uppercase tracking-widest active:scale-95 transition-transform"
          >
            Permitir
          </button>
        </div>
      ...
      {isSupported && !isSubscribed && profile.role === 'admin' && (
        <div className="bg-slate-800 text-white px-4 py-2 text-xs font-bold flex justify-between items-center shadow-md">
          ...
          <button 
            ...
            className="bg-orange-500 text-white px-3 py-1 rounded-full text-[10px] uppercase tracking-widest active:scale-95 transition-transform"
          >
            Permitir
          </button>
        </div>
          ...
          <nav className={`fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] md:max-w-lg rounded-[2.5rem] p-2 flex justify-between shadow-2xl z-50 border transition-all duration-500 ${
            ...
            {availableRoles.includes('bartender') && (
              <button
                ...
                className={`flex-1 flex flex-col items-center py-3 rounded-[2rem] transition-all ${
                  activeTab === 'bartender' ? 'text-white shadow-lg animate-scale-up' : isLight ? 'text-slate-500 hover:text-slate-900' : 'text-slate-500 hover:text-slate-300'
                }`}
                style={activeTab === 'bartender' ? { backgroundColor: primaryColor } : {}}
              >
                <GlassWater size={18} /><span className="text-[8px] font-black uppercase mt-1">Barra</span>
              </button>
            )}
            ...
            {availableRoles.includes('waiter') && (
              <button
                ...
                className={`flex-1 flex flex-col items-center py-3 rounded-[2rem] transition-all ${
                  activeTab === 'waiter' ? 'text-white shadow-lg animate-scale-up' : isLight ? 'text-slate-500 hover:text-slate-900' : 'text-slate-500 hover:text-slate-300'
                }`}
                                                   ...
                                                   return (
                                                       <div key={item.id || idx} className="flex justify-between items-center group">
                                                           <span className={`text-[11px] font-black tracking-tight flex items-center gap-2 ${
                                                               ...
                                                               <span className={`text-[9px] w-5 h-5 rounded-lg flex items-center justify-center ${isDone ? (isLight ? 'bg-slate-200 text-slate-400' : 'bg-slate-900 text-slate-700') : 'bg-orange-500/10 text-orange-500'}`}>
                                                                   {item.quantity}
                                                               </span>
